  ...
  return (
    ...
    ">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8
        /* Mobile: fill viewport height minus header (~64px) */
        h-[calc(100svh-64px)] sm:h-auto
      ...
      ">
        ...
        {/* ── 2×2 Grid ──
            Mobile  : grid-cols-2, fills remaining height via flex-1
            Desktop : grid-cols-2, fixed card height
        */}
        <div
          ref={gridRef}
          className="grid grid-cols-2 gap-2.5 sm:gap-8 flex-1 sm:flex-none pb-3 sm:pb-0"
        >
          {MOCK_CATEGORIES.map(category => (
            <Link
              key={category.id}
              to={`/subcategories?catId=${category.id}`}
              className="category-card group relative bg-white rounded-2xl sm:rounded-3xl overflow-hidden shadow-md hover:shadow-2xl border border-slate-200/80 transition-all duration-300 hover:-translate-y-1 h-full sm:h-72"
            >
              {/* Background Image */}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-8 text-white space-y-1 sm:space-y-2">
                {/* Product count badge — hidden on mobile to save space */}
                ...
                {/* Mobile: tiny product count */}
                <p className="sm:hidden text-brand-green text-[10px] font-bold">
                  {category.productCount}+ Products
                </p>
              </div>
            </Link>
